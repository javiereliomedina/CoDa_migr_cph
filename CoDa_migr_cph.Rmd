...
**Authors:** Javier Elío^[Department of Planning, Aalborg University, Copenhagen, Denmark; ORCID: [0000-0003-0624-2345](https://orcid.org/0000-0003-0624-2345); javierdem@plan.aau.dk - javiereliomedina@gmail.com],
Marina Georgati^[Department of Planning, Aalborg University, Copenhagen, Denmark; ORCID: [0000-0001-7794-8308](https://orcid.org/0000-0001-7794-8308); marinag@plan.aau.dk],
Henning S. Hansen^[Department of Planning, Aalborg University, Copenhagen, Denmark; ORCID: [0000-0001-7004-0698](https://orcid.org/0000-0001-7004-0698); hsh@plan.aau.dk],
Carsten Keßler^[Department of Planning, Aalborg University, Copenhagen, Denmark; ORCID: [0000-0002-9724-820X](https://orcid.org/0000-0002-9724-820X); kessler@plan.aau.dk].\
...
Ternary representation of population distribution. We made the ternary plots in a separate project ([https://javiereliomedina.github.io/ternary_maps_DK/](https://javiereliomedina.github.io/ternary_maps_DK/) because there are some conflicts between *ggtern (3.3.0)* and *ggplot2 (3.3.5)*). Here we only load the results. 

```{r fig-tern-pop, fig.cap = "Population distribution in 2020", fig.height=5, fig.width=5}

knitr::include_graphics("pop_tern.png")
...
Higher values of b1 indicate less non-Western population:

```{r fig-b1-non-wst, fig.height=3, fig.width=4.5, fig.cap = "Non-Western population vs. balance 1"}

cptl_prsh_ancestry_sf %>%
  ggplot() +
  geom_point(aes(x = b1,
                 y = pop_frgn_nwst_pct)) + 
  labs(y = "non-Western population [%]")
...
f_local_moran <- function(year, 
  ...
  xp %>% 
    # Plot quadrants (LISA)
    ggplot() +
    geom_sf(data = cptl_prsh, fill = "black") + 
...
as_tibble(sum_runits_oft_prices) %>%
...
Ternary diagram with house prices (zoom the figure to the parishes with median values). 

```{r fig-tern-pop-prices, fig.cap = "Ternary diagram of population distribution vs. housing prices", fig.height=5, fig.width=5}

knitr::include_graphics("pop_tern_prices.png")
...
# Link median prices (sum_runits_oft_prices) with population (balances)
...
# Model prices
m1 <- lm(log10(median_kDKK_m2) ~ b1 + b2, data = bal_prices)
...
summary(m1) %>% 
  prettify() %>% 
  kbl(caption = "Regression coefficients (β) of the log-linear model",
      digits = 3) %>%
  kable_styling()
...
ggplot() +
  ...
  scale_x_continuous(name = "Prediction [kDKK/m2]",
                     trans = "log10") +
  scale_y_continuous(name = "Median values [kDKK/m2]", 
                     trans = "log10") +
