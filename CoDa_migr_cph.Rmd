...
# Add transects
p + geom_line(data = tran,aes(x = dan,
                              y = wst,
                              z = nwst,
                              colour = name),
              size = 1) +
...
p1 <- tran %>% 
  ...
  labs(title = "A) Transect 1: log(Danes/Western) = 2.367",
       x = "nwst [%]",
       y = "log10(HP)")
...
p2 <- tran %>% 
  ...
  geom_line() +
  scale_y_continuous(limits = c(0, 3.5)) +
  labs(title = "B) Transect 2: log(Danes/non-Western) = 1.856",
       x = "wst [%]",
       y = "log10(HP)")
