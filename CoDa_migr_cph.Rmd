...
Ternary representation of population distribution. We made the ternary plots in a separate project (<https://javiereliomedina.github.io/ternary_maps_DK/> because there are some conflicts between *ggtern (3.3.0)* and *ggplot2 (3.3.5)*). Here we only load the results.
...
ggtern::ggtern(data = sum_runits_oft_prices,
               aes(x = pop_dan,
                   ...
                   colour = cut_number(median_kDKK_m2,
                                       n = 10,
                                       dig.lab = 0))) +
  scale_colour_manual(name = "Pred HP\n[kDkk/m2]",
                         values = rainbow(10)) +
...
lm1    <- lm(log10(median_kDKK_m2) ~ danwst_nwst  + dan_wst, data = dat_m1)
...
lm2    <- lm(log10(median_kDKK_m2) ~ dannwst_wst  + dan_nwst, data = dat_m2)
...
lm3    <- lm(log10(median_kDKK_m2) ~ nwstwst_dan  + nwst_wst, data = dat_m3)
...
```{r}
## Transects 
# Start in the mean composition (D = 80%, wst = 7.5%; nwst = 12.5%)

# HP variation with nwst when log-ratio(dan/wst) = c1
c1 <- log(80/7.5)
seg1 <- tibble(name = "seg1",
               nwst = seq(1, 99, 1),
               wst = (100 - nwst)/(1 + exp(c1)), 
               dan = 100 - nwst - wst)

# HP variation with wst when log-ratio(dan/nwst) = c2
c2 <- log(80/12.5)
seg2 <- tibble(name = "seg2",
               wst = seq(1, 99, 1),
               nwst = (100 - wst)/(1 + exp(c1)), 
               dan = 100 - nwst - wst)

# Combine 
seg <- bind_rows(seg1, seg2)
...
# Predictions
seg_pred <- predict(lm1, seg, se.fit = T, level = 0.95) %>% 
  as_tibble()

# Add predictions to the table
seg <- bind_cols(seg, seg_pred)

# Plot
ggtern::ggtern(data = seg,
                     aes(x = dan,
                         y = wst,
                         z = nwst,
                         col = name)) +
...
p1 <- seg %>% 
  ...
  labs(title = "A) Constant log-ratio Danes vs. Western")


p2 <- seg %>% 
  filter(name == "seg2") %>% 
  ggplot(aes(x = wst, y = fit)) + 
  geom_ribbon(aes(ymin = fit - 1.96*se.fit,
                  ymax = fit + 1.96*se.fit),
              alpha = 0.5,
              fill = "red") +
  geom_line() +
  scale_y_continuous(limits = c(0, 3.5)) +
  labs(title = "B) Constant log-ratio Danes vs. non-Western")
